...
def main(max_level):
    ...
    ╚══════╝╚═╝╚══════╝     ╚═════╝ ╚═════╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝
          """)
    print(Fore.GREEN + Style.BRIGHT + "Wormfare AUTO BOT")
    ...
    print(f"\n\n{Fore.CYAN+Style.BRIGHT}==============Find {total_accounts} Accounts=================\n")

    while True:
        for query_data in query_data_list:
            print(f"\n\n{Fore.CYAN+Style.BRIGHT}==============Accounts {akun}=================\n")
            akun += 1
            ...
            if response.status_code == 200:
                ...
                if token:
                    ...
                    if profile_response.status_code == 200:
                        ...
                        print(f"{Fore.BLUE}[ Profile ] : Nama Account: {profile_data.get('fullName')}")
                                    ...
                                    for booster in sorted_shop_data:
                                        ...
                                        if current_level < max_level_booster and current_level < max_level:
                                            ...
                                            if buyResponse.status_code == 200:
                                                print(f"{Fore.GREEN}[ Upgrade ] : Successful upgrade {upgrade_type} level of {current_level + 1}")
                                                shopResponse = requests.get(ShopUrl, headers=profile_headers)
                                                shop_data = shopResponse.json().get('availableBoost')
                                                upgrade = True
                                                break
                                            else:
                                                print(f"{Fore.RED}[ Upgrade ] : Gagal upgrade {upgrade_type} low balance")
                                        else:
                                            print(f"{Fore.YELLOW}[ Upgrade ] : {upgrade_type} already max level.")
                                else:
                                    print(f"{Fore.YELLOW}[ Upgrade ] : Stuck (excluding auto_bot).")
                            except json.JSONDecodeError as e:
                                print(f"{Fore.RED}[ ShopData ] : Failed to decode JSON response: {e}")
                        else:
                            print(f"{Fore.RED}[ ShopData ] : Failed to get shop data: {shopResponse.status_code}")
                    ...
                    if auto_booster == 'Y' and turbo > 0:
                        ...
                        if responseBooster.status_code != 200:
                            print(f"{Fore.RED}[ Booster ] : Failed to activate turbo!")
                        else:
                            print(f"{Fore.GREEN}[ Booster ] : Successfully activated turbo!")
                    ...
                    while energyLeft > 0:
                        ...
                        if slapResponse.status_code == 200:
                            print(f"{Fore.GREEN}[ Slap ] : Success Slap")
                        else:
                            print(f"{Fore.RED}[ Slap ] : Fail Slap")
                        ...
                        if energyLeft < 10 and recharge_slaps == 'Y' and full_energy > 0:
                            ...
                            if responseRechargeSlaps.status_code == 200:
                                print(f"\r{Fore.GREEN+Style.BRIGHT}[ Booster ] : Successfully recharged slaps!", flush=True)
                            else:
                                print(f"\r{Fore.RED+Style.BRIGHT}[ Booster ] : Gagal recharge slaps!", flush=True)
                            energyLeft = requests.get(ProfileUrl, headers=profile_headers).json().get('energyLeft')
                        if energyLeft > 10:
                            print(f"\r{Fore.GREEN+Style.BRIGHT}[ Slap ] : Energy is still available, slap again", flush=True)
                            continue
                        else:
                            print(f"\r{Fore.RED+Style.BRIGHT}[ Slap ] : Energy Out", flush=True)
                            time.sleep(5)
                            profile_data = profile_response.json()
                            resetPayload = {
                                "amount" : 1,
                    ...
                    print(f"{Fore.RED}[ Token ] : Failed to get Token!")
            else:
                print(f"{Fore.RED}[ Token ] : Login Failed!")
        
        
        print(f"\n\n{Fore.CYAN+Style.BRIGHT}==============All accounts have been processed=================\n")

        for detik in range(300, 0, -1):
            print(f"{Fore.YELLOW}[ !!!!!!!!!! ] : Re-slap inside {detik} second", end="\r", flush=True)
            time.sleep(1)
        print(f"{Fore.YELLOW}[ !!!!!!!!!! ] : Re-slap in 0 seconds", end="\r", flush=True)
        print("\n")
        akun = 1
                        

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Wormfare Auto Bot')
